...
**该修补程序是为Adobe Commerce版本创建的：** 云基础架构上的Adobe Commerce 2.3.6
...
1. 创建一个带有一个选项的可配置产品(例如： **颜色** = *红色*)并设置 **数量** = *1*.
1. 创建一个客户，然后订购此产品，此时此产品将变为 **数量** = *0*.
1. 请转到“管理员”，尝试重新排序前一个订单。

<u>预期结果</u>：

客户获得&quot;*此产品缺货。*”消息，表明产品缺货（预期）。

<u>实际结果</u>：

客户获得&quot;*此产品缺货。*”消息，以及 `var/log/exception.log` 包含类似的异常：
