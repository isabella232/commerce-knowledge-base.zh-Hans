...
ACSD-49574修补程序修复了无法通过GraphQL在购物车中更新礼品卡产品的问题。 此修补程序在以下情况下可用： [[!DNL Quality Patches Tool (QPT)]](/help/announcements/adobe-commerce-announcements/magento-quality-patches-released-new-tool-to-self-serve-quality-patches.md) 已安装1.1.28。 修补程序ID为ACSD-49574。 请注意，该问题计划在Adobe Commerce 2.4.7中修复。
...
1. 尝试使用通过GraphQL更新购物车中的礼品卡产品字段 `updateCartItems` 突变。
...
使用更新所有礼品卡产品字段(sender_name、sender_email、recipient_name、recipient_email、message、amount) `updateCartItems` 突变。
